...
class RocketsList: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    private func addDocument() {
        ...
        ref = db.collection("rockets").addDocument(data: [
            "rocketName": reuse.rocketNameArr[tag],
            "rocketFlightNumber": reuse.idArr[tag]
        ]) { err in
            if let err = err {
    ...
    private func updatedData() {
        // [START example_data]
        let rocketsRef = db.collection("rockets")
        
        for i in (0..<reuse.rocketNameArr.count).reversed() {
            
            rocketsRef.document("SF").setData([
                "rocketName": reuse.rocketNameArr[i],
                "rocketFlightNumber": reuse.idArr[i]
    ...
    @objc func connected(sender: UIButton){
        ...
        else
        {
            ...
            //            writeData(text: rocketNameArr[tag] as! String)
